...
所以当看门狗开始工作了的时候，就说明芯片已经不知道在干嘛了，反正没有在喂狗（feed），此时可以认为芯片已经跑飞了，程序已经没有在工作，没有绑定处理的中断函数的话，就只能重启了。

任何硬件产品都有可能出现意外和错误，看门狗相当于芯片上的最后一层保障机制，通常它可能会发生在函数栈的指针参数执行出错，导致后续的喂狗操作再也到不了，芯片程序进入空转，有些程序在设计上还会多了一层中断函数处理，可以让你试图纠正这种错误，或是把这些错误打印出来。
...
为什么会造成这种差异的原因是因为串口依次输入命令执行会存在误差，而 jupyter 是通过网络 socket 连接传输显示到屏幕上，所以耗时误差只会发生在运算重定向输出结果的时候，最终结果会较为贴近真实运算结果，通过保存下述代码文件来运行即可得知真实情况下约为 28ms 完成。
...
在日常中存在最多操作就是循环和判断，显然最好的优化就是减少不必要的指令操作，可以通过改变代码的执行结构来进行优化，下面就来具体分析吧。
...
事实上我并不鼓励用户在 Python 上使用多线程，因为全局解释器锁（GIL）的存在，CPython 解释器中执行的每一个 Python 线程，都会先锁住自己，以阻止别的线程执行。而 CPython 解释器会去轮询检查线程 GIL 的锁住情况，每隔一段时间，Python 解释器就会强制当前线程去释放 GIL，这样别的线程才能有执行的机会。总得来说 CPython 的实现决定了使用多线程并不会带来性能的提升，反而会带来线程安全的问题，尤其是需要线程资源同步了。
